...
规格展示名通过 `service_account.display_name` 快照取得，不读取当前 `service_duration_config` 覆盖历史含义；EXCHANGE、TEST、HISTORY_IMPORT 在账号写入时分别冻结来源批次或规格名称。
...
`POST /api/service-accounts/test-issues` 仅允许 `GLOBAL`。请求字段为 `requestId`、`companyId`、`quantity`、`specCode`、可选四位 `accountPrefix`（缺省为 `TEST`）。规格的 `displayName`、`serviceType`、`durationDays` 和 `accountSilenceDays` 由后端按当前 enabled 规格读取并冻结到测试下发批次；不会读取或修改 `company_exchange_config`，也不会创建兑换批次、兑换明细或消耗服务码。

`GET /api/service-accounts/test-issues/query?requestId={requestId}` 查询测试下发批次和成功后的 `{id, name}` CORS identity，不返回密码。查询使用独立静态路径以遵循项目现有权限 URL 约定。TEST 创建复用 CORS `POST /BaseUser/userInfo/add`，批次 API `requestId` 与 CORS requestId 分离；重试始终复用同一个 CORS requestId。CORS `code=0,data=null` 会进入结果窗口内重试，超过窗口转 `MANUAL_REVIEW`。

## 历史账号导入

`POST /api/service-accounts/history-imports` 仅允许 `GLOBAL`。请求字段为 `requestId`、`companyId`、`specCode` 和完整的 `accounts: [{id, name}]`。`specCode` 必须存在，允许选择 disabled 的历史规格；`displayName`、`serviceType`、`durationDays` 和 `accountSilenceDays` 规格快照由后端读取并冻结。导入不调用 CORS create、不创建 `cors_operation`、不消耗服务码，并在同一事务内完成批次和全部 `service_account` 入账。
