...
- `PUT /api/companies/parent/update?companyId=...`
- `GET /api/config/service-durations`、`POST /api/config/service-durations/create`、`PUT /api/config/service-durations/update?id=...`
- `GET /api/config/system-company`、`PUT /api/config/system-company/update`
- `GET /api/config/service-durations/detail?id=...`（配置中心详情）
- `GET /api/service-code-exchange-config`、`POST /api/service-code-exchange-config/configure`（经销商一次性兑换前缀配置）
- `GET /api/service-codes`、`GET /api/service-codes/detail?id=...`、`GET /api/service-codes/statistics`（列表支持分页、筛选与动态 `displayStatus=WAITING|EXPIRING|EXPIRED|PROCESSING|CONSUMED`）
- `POST /api/service-codes/transfers`
- `GET /api/service-codes/transfers/history?id=...`
...
- GET /api/service-code-generation-batches/detail?batchNo=... 与 GET /api/service-code-generation-batches?orderNo=... 查询支持批次追溯；GET /api/service-code-generations/orders/detail?orderNo=... 返回订单头及规格批次摘要。
