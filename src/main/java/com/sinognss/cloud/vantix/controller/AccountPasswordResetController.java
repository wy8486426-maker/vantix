...
public class AccountPasswordResetController {
    ...
    @PostMapping("/api/service-accounts/{serviceAccountId}/password/reset")
    public Object reset(@PathVariable Long serviceAccountId, @Valid @RequestBody ResetRequest request) {
        return CommonResultAdapter.success(reserveService.reserve(request.requestId(), serviceAccountId));
    }

    @GetMapping("/api/account-password-resets/{requestId}")
