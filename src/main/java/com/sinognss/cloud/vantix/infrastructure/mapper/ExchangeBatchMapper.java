...
@Mapper
public interface ExchangeBatchMapper extends BaseMapper<ExchangeBatch> {
    @Select("SELECT * FROM exchange_batch WHERE request_id = #{requestId} LIMIT 1")
    ExchangeBatch selectByRequestId(@Param("requestId") String requestId);
    ...
    @Update("UPDATE exchange_batch SET status = 'FAILED', last_error_code = #{errorCode}, "
    ...
    @Update("UPDATE exchange_batch SET status = 'MANUAL_REVIEW', last_error_code = #{errorCode}, "
            + "last_error_message = #{errorMessage}, updated_at = #{updatedAt} "
            + "WHERE id = #{id} AND status = 'PROCESSING'")
